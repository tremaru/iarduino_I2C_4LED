    ...
    j=Wire.endTransmission();                                                 //   Выполняем инициированную ранее передачу данных.
      if(j==0){                                                               //   Если передача выполнена успешно (0-передача успешна / 1 - переполнен буфер для передачи / 2 - получен NACK при передаче адреса / 3 - получен NACK при передаче данных / 4 - другая ошибка), то ...
        ...
        if(((k[2]>>1)==i) && (k[3]==0xC3 || k[3]==0x3C)){                     //   Если значение k[2] (без последнего бита) совпадает с адресом устройства i, а значение k[3] совпадает с идентификатором 0xC3 (модуль «Metro»), или 0x3С (модуль «Flash»), то ...
          ...
          if(k[0]==0x13){Serial.print(F("клавиатура"                )); }else //   Если в 0 элементе массива k хранится значение 0x13, значит это клавиатура.
          if(k[0]==0x14){Serial.print(F("мотор-редуктор с драйвером")); }else //   Если в 0 элементе массива k хранится значение 0x14, значит это мотор.
          if(k[0]==0x15){Serial.print(F("реле на 1 канал"           )); }else //   Если в 0 элементе массива k хранится значение 0x15, значит это электромеханическое реле на 1 канал.
          if(k[0]==0x16){Serial.print(F("LED индикатор на 4 разряда")); }else //   Если в 0 элементе массива k хранится значение 0x16, значит это 4-разрядный LED индикатор.
